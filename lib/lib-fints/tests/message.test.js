...
describe('Message', () => {
    describe('decoding', () => {
        it('with known segments', () => {
            const message = Message.decode("HNHBK:1:3+000000000108+300+0+1'HKIDN:2:2+280:12030000+9999999999+0+0'HKVVB:3:3+0+0+0+ABCDEF+1.0'HNHBS:4:1+1'");
            expect(message.segments.length).toBe(4);
        });
        it('with one unknown segment', () => {
            const message = Message.decode("HNHBK:1:3+000000000108+300+0+1'HKIDN:2:2+280:12030000+9999999999+0+0'HKVVB:3:3+0+0+0+ABCDEF+1.0'HIXXX:4:1+13'HNHBS:5:1+1'");
            expect(message.segments.length).toBe(5);
        });
        it('with signed and encrypted data', () => {
            const text = "HNHBK:1:3+000000000412+300+0+1'HNVSK:998:3+PIN:1+998+1+1+1:20240601:102151+2:2:13:@8@00000000:5:1+280:123456:user1:S:0:0+0'HNVSD:999:1+@259@HNSHK:2:4+PIN:2+944+1+1+1+1::rWkyTQV5yI8BAABFH0nthrBCCgQA+1+1:20240601:102151+1:3:1+6:10:16+280:30520037:user1:S:0:0'HKIDN:3:2+280:123456+user1+rWkyTQV5yI8BAABFH0nthrBCCgQA+1'HKVVB:4:3+43+9+0+9FA6681DEC0CF3046BFC2F8A6+0.1'HKTAN:5:6+4+HKIDN'HNSHA:6:2+1++12345''HNHBS:7:1+1'";
            const message = Message.decode(text);
            const hkidn = message.findSegment(HKIDN.Id);
    ...
    describe('findAllSegments()', () => {
        it('returns all segments with given ID', () => {
            ...
            const segments = message.findAllSegments(HIRMS.Id);
...
describe('CustomerMessage', () => {
    ...
    it('with no signature encodes correctly', () => {
        ...
        expect(encodedMessage).toBe("HNHBK:1:3+000000000108+300+0+1'HKIDN:2:2+280:12030000+9999999999+0+0'HKVVB:3:3+0+0+0+ABCDEF+1.0'HNHBS:4:1+1'");
    });
