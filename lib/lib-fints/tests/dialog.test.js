...
describe('Dialog', () => {
    ...
    beforeEach(() => {
        ...
        config = FinTSConfig.fromBankingInformation('TestProduct', '1.0', bankingInformation, 'testuser', '12345', 940, // tanMethodId
        'TAN-Generator 123');
    ...
    describe('constructor', () => {
        ...
        it('creates dialog with syncSystemId option', () => {
            ...
            const initDialogInteraction = syncDialog.interactions[0];
        ...
        it('throws error when no config is provided', () => {
            expect(() => new Dialog(null)).toThrow('configuration must be provided');
        });
    });
    ...
    describe('continue()', () => {
        ...
        it('throws error when tanOrderReference is missing', async () => {
            await expect(dialog.continue('')).rejects.toThrow('tanOrderReference must be provided to continue a customer order with a TAN');
        });
        it('throws error when TAN is missing for non-decoupled method', async () => {
            ...
            await expect(dialog.continue('TAN_REF_123')).rejects.toThrow('TAN must be provided for non-decoupled TAN methods');
        });
        it('throws error when dialog has ended', async () => {
            dialog.hasEnded = true;
            await expect(dialog.continue('TAN_REF_123', '123456')).rejects.toThrow('cannot continue a customer order when dialog has already ended');
        });
        it('throws error when no current interaction', async () => {
            dialog.currentInteractionIndex = dialog.interactions.length;
            await expect(dialog.continue('TAN_REF_123', '123456')).rejects.toThrow('there is no running customer interaction in this dialog to continue');
        });
    });
    describe('addCustomerInteraction()', () => {
        let sepaAccountInteraction;
        beforeEach(() => {
        ...
        it('throws error when dialog has ended', () => {
            dialog.hasEnded = true;
            expect(() => dialog.addCustomerInteraction(sepaAccountInteraction)).toThrow('cannot queue another customer interaction when dialog has already ended');
        });
        it('throws error for unsupported transaction', () => {
            // Mock unsupported transaction
            sepaAccountInteraction.segId = 'UNSUPPORTED';
            expect(() => dialog.addCustomerInteraction(sepaAccountInteraction)).toThrow('customer order transaction UNSUPPORTED is not supported according to the BPD');
        });
    });
});
